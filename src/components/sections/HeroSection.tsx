...
export function HeroSection({ settings }: HeroProps) {
  ...
  return (
    <section className="relative -mt-[calc(4rem+32px)] flex min-h-dvh items-center overflow-hidden lg:-mt-[calc(5rem+48px)]">
      ...
      <div className="bg-mesh-purple noise-overlay absolute inset-0" />
      <div className="bg-[radial-gradient(ellipse 80% 60% at 20% 50%,oklch(30% 0.18 290 / 0.3),transparent_70%),radial-gradient(ellipse 60% 80% at 80% 30%,oklch(22% 0.12 295 / 0.2),transparent_60%)] absolute inset-0" />

      {/* Ambient glow orbs */}
      <div className="pointer-events-none absolute -top-40 -right-40 h-[60vh] w-[60vh] rounded-full border border-white/5" />
      <div className="border-purple-light/5 pointer-events-none absolute -bottom-40 -left-40 h-[50vh] w-[50vh] rounded-full border" />
      ...
      <div className="relative z-10 mx-auto w-full max-w-[var(--max-width)] px-6 lg:px-10">
        <div className="grid items-center gap-16 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="max-w-xl lg:pl-6">
            ...
            >
              <span className="bg-gold/70 h-px w-14" />
              <span className="text-gold/60 text-[10px] tracking-[0.4em] uppercase">
                {brand.siteName}
              </span>
            </div>
            ...
            <h1
              className="hero-home-title text-cream leading-[0.88] tracking-[-0.04em] text-[var(--text-hero)]"
              style={{
                animation: mounted
                  ? `fadeUp 0.8s 0.15s var(--ease-out) both`
                  : "none",
              }}
            >
            ...
            {/* Divider */}
            <div
              className="from-gold/80 to-gold/10 my-10 h-px w-28 bg-gradient-to-r"
              style={{ animation: mounted ? `fadeIn 0.6s 0.35s both` : "none" }}
            />

            {/* Description */}
            <p
              className="text-beige/60 text-lg leading-9 text-balance lg:text-xl lg:leading-10"
              style={{
                animation: mounted
                  ? `fadeUp 0.8s 0.4s var(--ease-out) both`
                  : "none",
              }}
            >
            ...
            <div
              ...
              style={{
                animation: mounted
                  ? `fadeUp 0.8s 0.55s var(--ease-out) both`
                  : "none",
              }}
            >
          ...
          <div
            ...
            style={{
              animation: mounted
                ? `reveal-wipe 1.2s 0.3s var(--ease-out) both`
                : "none",
            }}
          >
            <div className="bg-surface/5 overflow-hidden rounded-[3.5rem] p-3 shadow-[0_32px_80px_rgba(0,0,0,0.2)]">
              <div className="relative aspect-[4/5] overflow-hidden rounded-[2.8rem]">
                <Image
                  ...
                  className="scale-110 object-cover transition-all duration-[2.5s] hover:scale-100"
                  priority
                />
