...
export function AdminSideNav({ items }: { items: readonly NavItem[] }) {
  ...
  return (
    <nav className="grid gap-1" aria-label="التنقل الإداري">
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            <span className="min-w-0 pt-1">
              <span className={`block font-semibold tracking-tight ${active ? "text-ink-strong" : "text-ink group-hover:text-ink-strong"}`}>
                {item.label}
              </span>
              <span className="mt-0.5 block text-[11px] leading-snug text-ink-faint group-hover:text-ink-soft">{item.description}</span>
            </span>
            {active ? (
              <span className="pointer-events-none absolute start-0 top-1/2 h-8 w-1 -translate-y-1/2 rounded-full bg-accent" />
            ) : null}
          </Link>
        );
      })}
